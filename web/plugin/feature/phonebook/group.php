...
switch (_OP_) {
	case "list":
		...
		for ($j = 0; $j < count($list); $j++) {
			...
			$conditions = array(
				...
				$keywords = $search['dba_keywords'];
				$gcount = dba_count(_DB_PREF_ . '_featurePhonebook_group_contacts', $conditions, $keywords);
			...
			$content .= "
				<tr>
					<td><a href='" . _u('index.php?app=main&inc=feature_phonebook&route=group&op=edit&gpid=' . $gpid) . "'>" . $name . "</a><td><span class=\"badge\">".$gcount."</span></td></td>
	...
	case "edit":
		...
		$gname = phonebook_groupid2name($user_config['uid'], $gpid);
		$gid = phonebook_hook_phonebook_groupname2id($user_config['uid'], $gname);
		$gcontacts =phonebook_hook_phonebook_getdatabyid($gid);
		$allcontacts = phonebook_hook_phonebook_getallcontactsbyid($gid);
		$base_url = 'index.php?app=main&inc=feature_phonebook&route=group&op=edit&gpid='.$gpid;
		$search = themes_search($search_category, $base_url);
		$conditions = array(
			'uid' => $user_config['uid'] 
		);
		$keywords = $search['dba_keywords'];
		$count = count($gcontacts);
		//var_dump($search['url']);
		$nav = themes_nav($count, $search['url']);
		$extras = array(
			'ORDER BY' => 'name',
		...
		$content = "
			...
			<div class=\"col-sm-5\">
			<h3>" . _('أعضاء المجموعة') . "</h3>
			<form name=fm_group_list id=fm_group_list action='index.php?app=main&inc=feature_phonebook&op=actions&go=edit' method=post>
			" . _CSRF_FORM_ . "
			...
			<tr>
				<th width=30%>" . _('Name') . "</th>
				<th width=30%>" . _('Mobile') . "</th>
				<th width=5%><input type=checkbox onclick=CheckUncheckAll(document.fm_group_list)></th>
			</tr>
			<tr>
			</tr>
			</thead>
			...
			$i = $nav['top'];
			$j = 0;
			for ($j = 0; $j < count($gcontacts); $j++) {
			$pid = $gcontacts[$j]['pid'];
			$name = $gcontacts[$j]['p_desc'];
			$mobile = $gcontacts[$j]['p_num'];
			$i--;
		...
		<div class=\"col-sm-5\">
			<h3>" . _('كافة جهات الاتصال') . "</h3>
			<form name=fm_allcontacts_list id=fm_allcontacts_list action='index.php?app=main&inc=feature_phonebook' method=post>
			" . _CSRF_FORM_ . "
			<input type=hidden id=action_route name=route value=''>
			<div class=table-responsive>
			<table class=playsms-table-list>
			...
			<tr>
				<th width=30%>" . _('Name') . "</th>
				<th width=30%>" . _('Mobile') . "</th>
				<th width=5%><input type=checkbox onclick=CheckUncheckAll(document.fm_allcontacts_list)></th>
			</tr>
			<tr>
			</tr>
			</thead>
			...
			$pid = $allcontacts[$j]['pid'];
			$name = $allcontacts[$j]['p_desc'];
			$mobile = $allcontacts[$j]['p_num'];
			$i--;
			$c_i = "<a href=\"" . _u('index.php?app=main&inc=feature_phonebook&op=phonebook_edit&id=' . $pid) . "\">" . $i . ".</a>";
			//var_dump($i--);
			$content .= "
