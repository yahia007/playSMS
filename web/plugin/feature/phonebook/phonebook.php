...
switch (_OP_) {
	case "phonebook_list":
		...
		foreach ($phonebook_groups as $group) {
			$action_move_options .= '<option value=move_' . $group['gpid'] . '>' . _('Move to') . ' ' . $group['gp_name'] . ' (' . $group['gp_name'] . ')</option>';
		}
		...
		for ($j = 0; $j < count($list); $j++) {
			...
			$tags = $list[$j]['tags'];
			$group_code = "";
			$groupfields = 'B.id AS id, B.uid AS uid, B.name AS name, B.flag_sender AS flag_sender';
			...
			for ($k = 0; $k < count($grouplist); $k++) {
				if ($grouplist[$k]['uid'] == $user_config['uid']) {
					$group_code .= $phonebook_flag_sender[$grouplist[$k]['flag_sender']] . "<a href=\"" . _u('index.php?app=main&inc=feature_phonebook&route=group&op=edit&gpid=' . $grouplist[$k]['id']) . "\">" . strtoupper($grouplist[$k]['name']) . "</a><br />";
				} else {
					$group_code .= $phonebook_flag_sender[$grouplist[$k]['flag_sender']] . strtoupper($grouplist[$k]['name']) . "<br />";
				}
			} 
			$i--;
			...
			if ($list[$j]['uid'] == $user_config['uid']) {
				...
				$send = _sendsms($mobile, '', 'index.php?app=main&inc=feature_phonebook&op=phonebook_list', $icon_config['send']);
			}
			$content .= "
				<tr>
					<td>$name<div id=\"msg_option\" style=\"float:left;\">$send</div></td>
					<td>$mobile</td>
					<td>$email</td>
					<td>$group_code</td>
					<td>$tags</td>
