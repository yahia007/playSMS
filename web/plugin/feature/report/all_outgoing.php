...
switch (_OP_) {
	case "all_outgoing":
		...
		if ($queue_code = trim($_REQUEST['queue_code'])) {
			$queue_label = "<h4>" . sprintf(_('عرض طابور الإرسال رقم %s'), $queue_code) . "</h4>";
			...
			$list = dba_search($table . ' AS A', 'B.username, A.p_gateway, A.p_smsc, A.smslog_id, A.p_dst, A.p_sms_type, A.p_msg, A.p_src, A.p_footer, A.p_datetime, A.p_update, A.p_status, A.uid, A.queue_code', $conditions, $keywords, $extras, $join);
		} else {
			$search = themes_search($search_category, $base_url);
			$conditions = array(
				'A.flag_deleted' => 0 
			...
			$list = dba_search($table . ' AS A', 'B.username, A.p_gateway, A.p_smsc, A.smslog_id, A.p_dst, A.p_sms_type, A.p_msg, A.p_src, A.p_footer, A.p_datetime, A.p_update, A.p_status, A.uid, A.queue_code, COUNT(*) AS queue_count', $conditions, $keywords, $extras, $join);
		}
		...
		for ($j = 0; $j < count($list); $j++) {
			...
			$queue_view_link = "";
			if ($c_queue_count > 1) {
				$queue_view_link = "<a href='" . $base_url . "&queue_code=" . $c_queue_code . "'>" . sprintf(_(' عرض الكل (%d)'), $c_queue_count) . "</a>";
			}
			// 0 = pending
			// 1 = sent
			// 2 = failed
