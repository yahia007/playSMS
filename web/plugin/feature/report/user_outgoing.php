...
switch (_OP_) {
	case "user_outgoing":
		...
		if ($queue_code = trim($_REQUEST['queue_code'])) {
			...
			$list = dba_search($table . ' AS A', 'A.smslog_id, A.p_dst, A.p_sms_type, A.p_msg, A.p_src, A.p_footer, A.p_datetime, A.p_update, A.p_status, A.uid, A.queue_code', $conditions, $keywords, $extras, $join);
		} else {
			$search = themes_search($search_category, $base_url);
			$conditions = array(
				'A.uid' => $user_config['uid'],
			...
			$list = dba_search($table . ' AS A', 'A.smslog_id, A.p_dst, A.p_sms_type, A.p_msg, A.p_src, A.p_footer, A.p_datetime, A.p_update, A.p_status, A.uid, A.queue_code, COUNT(*) AS queue_count', $conditions, $keywords, $extras, $join);
		}
		
		$content = "
			<h2>" . _('رسائلي المرسلة') . "</h2>
			" . $queue_label . "
			<p>" . $search['form'] . "</p>
			...
			<div class=actions_box>
				...
				<div class=pull-right>
					<a href='#' onClick=\"return SubmitConfirm('" . _('هل أنت متأكد من حذف العناصر المحددة؟') . "', 'fm_user_outgoing');\">" . $icon_config['delete'] . "</a>
				</div>
			</div>
			...
			<thead>
			<tr>
				<th width=20%>" . _('إلى') . "</th>
				<th width=75%>" . _('الرسالة') . "</th>
		...
		for ($j = 0; $j < count($list); $j++) {
			...
			// 3 = delivered
			if ($p_status == "1") {
				$p_status = "<span class=status_sent title='تم الإرسال'>تم الإرسال<span/>";
			} else if ($p_status == "2") {
				$p_status = "<span class=status_failed title='فشل'>فشل<span/>";
			} else if ($p_status == "3") {
				$p_status = "<span class=status_delivered title='تم التسليم'>تم التسليم<span/>";
			} else {
				$p_status = "<span class=status_pending title='قيد الإرسال'>قيد الإرسال<span/>";
			}
	...
	case "actions":
		...
		switch ($go) {
			...
			case 'delete':
				...
				$_SESSION['dialog']['info'][] = _('تم حذف الرسائل المحددة');
