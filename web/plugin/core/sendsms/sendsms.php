...
switch (_OP_) {
	case "sendsms":
		...
		$sender = stripslashes($_REQUEST['from']);
		...
		// message template
		$option_values = "<option value=\"\" default>--" . _('الرجاء اختيار قالب') . "--</option>";
		...
		$tpl = array(
			'name' => $layout,
			'vars' => array(
				'Compose message' => _('إنشاء رسالة'),
				'Sender ID' => _('اسم المرسل'),
				'Message footer' => _('التذييل'),
				'Send to' => _('إرسال إلى'),
				'Message' => _('الرسالة'),
				'Flash message' => _('رسالة فلاش'),
				'Unicode message' => _('رسالة يونيكود'),
				'Send' => _('إرسال'),
				'Cancel' => _('إلغاء'),
				'Schedule' => _('جدولة'),
				'Options' => _('خيارات'),
				...
				'HINT_SEND_TO' => _('استخدم # لاختيار مجموعة و@ لاختيار مستخدم'),
				'HINT_SCHEDULE' => _('الصيغة YYYY-MM-DD hh:mm'),
				'sms_from' => $sms_from,
	...
	case "sendsms_yes":
		...
		if ($sms_to[0] && $message) {
			...
			if (!$sms_count && $sms_failed) {
				$_SESSION['dialog']['danger'][] = _('فشل الإرسال لكافة الجهات') . " (" . _('الطابور') . ":" . (int) $sms_count . " " . _('فشل') . ":" . (int) $sms_failed . ")";
			} else if ($sms_count && $sms_failed) {
				$_SESSION['dialog']['danger'][] = _('تم الإرسال إلى بعض الجهات') . " (" . _('الطابور') . ":" . (int) $sms_count . " " . _('فشل') . ":" . (int) $sms_failed . ")";
			} else if ($sms_count && !$sms_failed) {
				$_SESSION['dialog']['info'][] = _('تم وضع الرسالة في الطابور') . " (" . _('الطابور') . ":" . (int) $sms_count . " " . _('فشل') . ":" . (int) $sms_failed . ")";
			} else {
				if (!is_array($error_strings)) {
					$_SESSION['dialog']['danger'][] = $error_strings;
				} else {
					$_SESSION['dialog']['danger'][] = _('خطأ في النظام');
				}
			}
		} else {
			$_SESSION['dialog']['danger'][] = _('يجب اختيار مستلم ولايمكن ترك نص الرسالة فارغ!');
		}
